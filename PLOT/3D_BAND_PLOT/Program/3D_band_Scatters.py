...
# 绘制E_upp能量的散点图，使用反转后的红色映射表
sc1 = ax.scatter(kx, ky, E_upp, c=E_upp, cmap=cmap_red_reversed, depthshade=True)
# 绘制E_dnn能量的散点图，使用反转后的蓝色映射表
sc2 = ax.scatter(kx, ky, E_dnn, c=E_dnn, cmap=cmap_blue_reversed, depthshade=True)

# 添加颜色条，将值映射到颜色
cbar1 = fig.colorbar(sc1, ax=ax, pad=0.1, label='E_up (eV)')
cbar2 = fig.colorbar(sc2, ax=ax, pad=0.1, label='E_down (eV)')
